...
export default function CoursesList() {
  ...
  return (
    <section className="relative bg-gray-50 py-24 px-6 overflow-hidden">
      ...
      <div className="absolute top-0 left-1/2 transform -translate-x-1/2 w-[800px] h-[600px] bg-yellow-100/40 rounded-full filter blur-3xl -z-10"></div>
      <div className="absolute bottom-0 right-1/2 transform translate-x-1/2 w-[600px] h-[400px] bg-yellow-300/30 rounded-full filter blur-3xl -z-10"></div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-10">
          {courses.map((course, index) => (
            <div
              ...
              className="p-6 bg-white rounded-3xl shadow-lg hover:shadow-2xl transform hover:-translate-y-2 transition-all duration-500 flex flex-col justify-between border border-gray-200"
            >
              <div>
                {/* Icon */}
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  <span className="bg-yellow-100 text-yellow-800 text-sm px-3 py-1 rounded-full font-medium animate-pulse">
                    🕒 {course.duration}
                  </span>
                  {course.extra && (
                    <span className="bg-gray-100 text-gray-800 text-sm px-3 py-1 rounded-full font-medium animate-pulse">
                      📌 {course.extra}
                    </span>
                  )}
                </div>
                ...
                {/* Outcome */}
                <p className="text-gray-700 font-medium mb-4">🎯 Outcome: {course.outcome}</p>
              </div>

              {/* CTA Button */}
              <a
                href="#"
                className="mt-auto inline-flex items-center justify-center gap-2 px-6 py-3 bg-yellow-400 text-black font-semibold rounded-xl shadow hover:bg-yellow-500 hover:shadow-lg transition text-center"
              >
                View Details <HiArrowRight size={20} />
              </a>
            </div>
          ))}
        </div>
